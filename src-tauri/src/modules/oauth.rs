...
const CLIENT_ID_ENV: &str = "ANTIGRAVITY_GOOGLE_OAUTH_CLIENT_ID";
const CLIENT_SECRET_ENV: &str = "ANTIGRAVITY_GOOGLE_OAUTH_CLIENT_SECRET";
...
fn client_id() -> &'static str {
    CLIENT_ID
        .get_or_init(|| std::env::var(CLIENT_ID_ENV).unwrap_or_default())
        .as_str()
}

fn client_secret() -> &'static str {
    CLIENT_SECRET
        .get_or_init(|| std::env::var(CLIENT_SECRET_ENV).unwrap_or_default())
        .as_str()
}

fn ensure_oauth_configured() -> Result<(), String> {
    if client_id().trim().is_empty() || client_secret().trim().is_empty() {
        return Err(format!(
            "Google OAuth credentials are not configured. Set {} and {}.",

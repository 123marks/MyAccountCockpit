...
const GEMINI_OAUTH_CLIENT_ID_ENV: &str = "GEMINI_GOOGLE_OAUTH_CLIENT_ID";
const GEMINI_OAUTH_CLIENT_SECRET_ENV: &str = "GEMINI_GOOGLE_OAUTH_CLIENT_SECRET";
...
static GEMINI_OAUTH_CLIENT_ID: OnceLock<String> = OnceLock::new();
static GEMINI_OAUTH_CLIENT_SECRET: OnceLock<String> = OnceLock::new();
...
pub fn gemini_oauth_client_id() -> &'static str {
    GEMINI_OAUTH_CLIENT_ID
        .get_or_init(|| std::env::var(GEMINI_OAUTH_CLIENT_ID_ENV).unwrap_or_default())
        .as_str()
}

pub fn gemini_oauth_client_secret() -> &'static str {
    GEMINI_OAUTH_CLIENT_SECRET
        .get_or_init(|| std::env::var(GEMINI_OAUTH_CLIENT_SECRET_ENV).unwrap_or_default())
        .as_str()
}

fn ensure_gemini_oauth_configured() -> Result<(), String> {
    if gemini_oauth_client_id().trim().is_empty()
        || gemini_oauth_client_secret().trim().is_empty()
    {
        return Err(format!(
            "Gemini Google OAuth credentials are not configured. Set {} and {}.",
            GEMINI_OAUTH_CLIENT_ID_ENV, GEMINI_OAUTH_CLIENT_SECRET_ENV
        ));
    }
    Ok(())
}
...
fn build_auth_url(callback_url: &str, state_token: &str) -> Result<String, String> {
    ensure_gemini_oauth_configured()?;
